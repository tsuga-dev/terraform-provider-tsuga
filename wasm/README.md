...
The provider repository is the source of truth. On pushes to `main`, CI builds
the package and synchronizes `dist/`, `package.json`, this README, and the
license to the public `tsuga-dev/terraform-wasm` repository. The publish job
requires `PAT_TO_WASM_PACKAGES_REPO` with write access to that repository.
